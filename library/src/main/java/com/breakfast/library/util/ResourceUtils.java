...
public class ResourceUtils {
  ...
  public static int getResourceId(String name, String type) {
    ...
    try {
      ...
      return resources.getIdentifier(name, type, BaseApplication.getInstance().getPackageName());
    } catch (Exception e) {
      e.printStackTrace();
    }
    return 0;
  ...
  public static byte[] readBytesFromAssets(String fileName) {
    ...
    byte[] buffer = null;
    try {
      is = BaseApplication.getInstance().getAssets().open(fileName);
  ...
  public static byte[] readBytesFromRaw(@RawRes int rawId) {
    ...
    byte[] buffer = null;
    try {
      is = BaseApplication.getInstance().getResources().openRawResource(rawId);
      int size = is.available();
      buffer = new byte[size];
      is.read(buffer);
    } catch (IOException e) {
  ...
  public static String getString(@StringRes int strId, Object... args) {
    return BaseApplication.getInstance().getResources().getString(strId, args);
  }
  ...
  public static ColorStateList getColorStateList(@DrawableRes int selectorId) {
    return ContextCompat.getColorStateList(BaseApplication.getInstance(), selectorId);
  }
  ...
  public static int getDimen(@DimenRes int dimenId) {
    return BaseApplication.getInstance().getResources().getDimensionPixelSize(dimenId);
  }
  ...
  public static String[] getStringArray(@ArrayRes int arrayId) {
    return BaseApplication.getInstance().getResources().getStringArray(arrayId);
  }
}
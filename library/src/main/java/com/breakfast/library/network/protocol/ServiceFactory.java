...
public final class ServiceFactory {
    ...
    public static <T> T generateService(Class<T> clazz) {
        synchronized (ServiceFactory.class) {
            return generateService(BaseApplication.getInstance(), clazz);
        }
    }

    public static <T> T generateService(Context applicationContext, Class<T> clazz) {
        synchronized (ServiceFactory.class) {
            if (sRetrofit == null) {
                sRetrofit = getRetrofit(applicationContext);
            }
    ...
    private static Retrofit getRetrofit(Context context) {
        ...
        if (BuildConfig.DEBUG) {
            baseUrl = SharedPreferenceUtils.getString(R.string.host);
            if (TextUtils.isEmpty(baseUrl)) {
                baseUrl = AppUtils.getMetaValue(context, MetaDataKeys.HOST);
            }
        } else {
            baseUrl = AppUtils.getMetaValue(context, MetaDataKeys.HOST);
        }
    ...
    public static synchronized Gson getGson() {
        if (gson == null) {
            ...
            gson = new GsonBuilder().registerTypeAdapter(Date.class, dateJsonDeserializer)
                    .registerTypeAdapter(java.util.Date.class, dateJsonSeserializer)
                    .setLenient()
                    // 反序列化时不忽略任何字段
